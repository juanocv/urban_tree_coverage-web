...
const DICT = {
  pt: {
    ...
    "status.unauthorized": "Token necessário",

    "conn.legend": "Conexão com a API",
    "conn.baseUrl": "Endereço da API",
    "conn.open": "Configurar a conexão com a API",
    "conn.change": "Alterar",
    "conn.changeLocked": "Endereço definido em UC_WEB_API_BASE_URL. Clique em Alterar para editar só neste navegador.",
    "conn.changeUnlocked": "Editando o endereço. Ele vale só neste navegador; o padrão publicado não muda.",
    "conn.token": "Token de acesso",
    "conn.tokenPlaceholder": "deixe vazio se a API não exigir",
    "conn.tokenHint": "Enviado como <code>Authorization: Bearer</code>. Fica só neste navegador — nunca no repositório nem no link compartilhável.",
    "conn.tokenReveal": "Mostrar ou ocultar o token",
    ...
    "model.backend": "Backend de segmentação",
    "model.backendDefault": "Padrão da instância",
    "model.backendHint": "Cada backend prevê um espaço de classes diferente. Conecte-se à API para ver quais estão disponíveis.",
    "model.backendUnavailable": "indisponível",
    "model.backendSpace": "Espaço de classes: {space}.",
    "model.backendNoTree": "Este espaço de classes não expressa árvores: sem o proxy de vegetação, a cobertura arbórea é reportada como indisponível.",
    "model.backendHasTree": "Este espaço de classes tem uma classe de árvore própria.",
    "model.backendOffline": "Lista provisória: só uma conexão bem-sucedida confirma o que a instância realmente oferece.",
    ...
    "model.overlaysHint": "RGB, máscara e sobreposição em PNG. Na multivista, um conjunto por direção.",
    "model.overlaysCost": "Cada direção acrescenta alguns megabytes à resposta. A API recusa planos acima de {max} direções com imagens; ajuste UC_API_MAX_OVERLAY_VIEWS se precisar de mais.",
    ...
    "err.unauthorized": "A API recusou o token. Confira o valor no campo Token de acesso.",
    "err.tokenRequired": "Esta API exige autenticação. Informe o token de acesso.",
    ...
    "res.imagery": "Imagens",
    "res.view": "Vista",
    "res.imageryByHeading": "Imagens por direção",
    "res.imageryHint": "Todas as miniaturas mostram a mesma camada; troque nas abas para comparar direções. Clique numa delas para ampliar.",
    "res.imageryNoneMulti": "Esta resposta não traz imagens. Ative <strong>Retornar imagens sobrepostas</strong> antes de analisar.",
    "res.openView": "Ampliar a vista de {heading}",
    "res.closeView": "Fechar",
    ...
    "help.auth": "Para acessar de qualquer lugar, ligue a autenticação e exponha a API por HTTPS. O Tailscale Funnel dá um endereço fixo de graça; o Cloudflare Tunnel faz o mesmo num domínio seu:",
    "help.authHint": "Cole o token no campo <strong>Token de acesso</strong>. Ele fica só neste navegador.",
    "help.key": "A API chama uma API paga do Google a cada requisição. Sem <code>UC_API_TOKENS</code> ela não tem autenticação nenhuma: mantenha-a em localhost ou defina um token antes de expô-la.",
  ...
  en: {
    ...
    "status.unauthorized": "Token required",

    "conn.legend": "API connection",
    "conn.baseUrl": "API address",
    "conn.open": "Configure the API connection",
    "conn.change": "Change",
    "conn.changeLocked": "Address set by UC_WEB_API_BASE_URL. Press Change to edit it for this browser only.",
    "conn.changeUnlocked": "Editing the address. It applies to this browser only; the published default is unchanged.",
    "conn.token": "Access token",
    "conn.tokenPlaceholder": "leave empty if the API needs none",
    "conn.tokenHint": "Sent as <code>Authorization: Bearer</code>. It stays in this browser only — never in the repository or the shareable link.",
    "conn.tokenReveal": "Show or hide the token",
    ...
    "model.backend": "Segmentation backend",
    "model.backendDefault": "Instance default",
    "model.backendHint": "Each backend predicts a different class space. Connect to the API to see which ones it offers.",
    "model.backendUnavailable": "unavailable",
    "model.backendSpace": "Class space: {space}.",
    "model.backendNoTree": "This class space cannot express trees: without the vegetation proxy, tree coverage is reported as unavailable.",
    "model.backendHasTree": "This class space has a tree class of its own.",
    "model.backendOffline": "Provisional list: only a successful connection confirms what the instance actually offers.",
    ...
    "model.overlaysHint": "RGB, mask and overlay as PNG. In multi-view, one set per heading.",
    "model.overlaysCost": "Each heading adds a few megabytes to the response. The API refuses plans above {max} headings with imagery; raise UC_API_MAX_OVERLAY_VIEWS if you need more.",
    ...
    "err.unauthorized": "The API rejected the token. Check the value in the Access token field.",
    "err.tokenRequired": "This API requires authentication. Enter the access token.",
    ...
    "res.imagery": "Imagery",
    "res.view": "View",
    "res.imageryByHeading": "Imagery by heading",
    "res.imageryHint": "Every thumbnail shows the same layer; switch tabs to compare headings. Click one to enlarge.",
    "res.imageryNoneMulti": "This response carries no imagery. Turn on <strong>Return overlay images</strong> before analysing.",
    "res.openView": "Enlarge the {heading} view",
    "res.closeView": "Close",
    ...
    "help.auth": "To reach it from anywhere, turn on authentication and expose the API over HTTPS. Tailscale Funnel gives a fixed address for free; Cloudflare Tunnel does the same on a domain you own:",
    "help.authHint": "Paste the token into the <strong>Access token</strong> field. It stays in this browser only.",
    "help.key": "The API calls a paid Google API on every request. Without <code>UC_API_TOKENS</code> it has no authentication at all: keep it on localhost, or set a token before exposing it.",
